...
const Setorder = (props) => {
  ...
  const [p2pHis, setP2pHis] = React.useState([]);
  ...
  const getP2PHistory = async () => {
    try {
      ...
      let config = {
        method: "POST",
        url: `${URL}/getUserP2pOrderHistory`,
        headers: {
          "Content-Type": "application/json",
          'x-access-token': `${props.token}`
        },
  ...
  return (
    <>
      ...
      <div className="balanceSheet">
        <div>
          <span  className="quickbsret" 
           onClick={(e) => {
            handleSidOpen();

          }}
          >P2P</span>
           <span className="quickbsret"   onClick={(e) => {
            getP2PHistory();

          }}>P2P HISTORY</span>
      </div>
        <h1 className="balance" style={{ paddingLeft: "15px" }}>
        {(historyOpen)?"P2P Order History":"P2P Order"}
        </h1>
          <div className="balanceSheet_div" id="popularoins">
          {(
            <DataGrid
              rows={(historyOpen)?p2pHis:p2porder}
              columns={(historyOpen)?columnsHistory:columns}
              pageSize={10}
