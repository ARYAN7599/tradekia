    ...
    axios(config)
      .then(res => {
        ...
        if (res.data.status) {
          ...
          setTrxprize(res?.data?.data?.Inrprize);
          setUsdtPrize(res?.data?.data?.UsdtPrize);
        }
      }).catch(error => {
        console.log("error", error);
      });
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (params) => {
        ...
        return (
          (ch24h.slice(0, 1) == "-") ?
          <span style={{ color: '#d9442e',minWidth:'125.855px',
          maxWidth:'171.855px',
          lineHeight:"51px",
          minHeight:'52px',
          maxHeight: '52px'}}>
          ...
          <span style={{ color: '#76b840',minWidth:'125.855px',
          maxWidth:'171.855px',
          lineHeight:"51px",
          minHeight:'52px',
          maxHeight: '52px' }}>
        ...
        return (
          <Box sx={{
            ...
            {(params.row.pair === "USDT") ?
              <span style={{ cursor: "pointer" }} onClick={(e) => { tradeHandler(e, params.row.coin, params.row.network); }} >Trade</span>

              : <>
                {/* <span style={{ color: '#29e70c' }} onClick={handleClickOpen}><SwapVerticalCircleIcon style={{ fontSize: "small", color: "#29e70c" }} />Deposit</span> */}
                <span style={{ color: '#29e70c', cursor: "pointer" }} onClick={(e) => { handleClickOpen(e, params.row.coin, params.row.network); }}>QuickBuy</span>
      ...
      <h1 className="balance">
        ...
        <div className="balanceSheet_div">
          ...
          <BootstrapDialog
            ...
            open={open}
          >
            <BootstrapDialogTitle id="customizeddd-dialog-title" onClose={handleClose}>
              {/* Add money to wallet */}
              {fromcoin}
            ...
            <DialogContent dividers>
              <Typography gutterBottom>
                ...
                <span style={{ paddingLeft: "5px" }}>  Amount You Pay : ₹ {orderAmount}</span>
              </Typography>
              <Typography gutterBottom>
                <Box sx={{ display: 'flex', flexWrap: 'wrap' }}>
                  <div>
                    <FormControl fullWidth sx={{ m: 1 }}>
                      <InputLabel htmlFor="outlined-adornment-amount">Amount(INR)</InputLabel>
                      <OutlinedInput
                        id="outlined-adornment-amount"
                        type="number"
                        value={orderAmount}
                        onChange={handleWalletChange}
                        endAdornment={<InputAdornment position="end">Min ₹ 1000 - Max ₹ 5L</InputAdornment>}
                        startAdornment={<InputAdornment position="start">₹</InputAdornment>}
                        label="Amount"
                      />
                    </FormControl>
                    <Box sx={{ display: 'flex' }}>
                      <Button sx={{ cursor: "pointer" }} onClick={(e) => { limtQpHandler(e, 1000); }} id="outlined-weight-helper-text">+₹ 1000</Button>
                      <Button sx={{ cursor: "pointer" }} onClick={(e) => { limtQpHandler(e, 5000); }} id="outlined-weight-helper-text">+₹ 5000</Button>
                      <Button sx={{ cursor: "pointer" }} onClick={(e) => { limtQpHandler(e, 10000); }} id="outlined-weight-helper-text">+₹ 10000</Button>

                      {/* <Button onClick={(e) => { limtQpHandler(e, 1000); }} id="outlined-weight-helper-text">+1000</Button> */}
                    </Box>
                    <FormControl fullWidth sx={{ m: 1 }}>
                      <InputLabel htmlFor="outlined-adornment-amount">Coins You Get</InputLabel>
                      <OutlinedInput
                        id="outlined-adornment-amount"
                        type="number"
                        startAdornment={<InputAdornment position="start">{parseFloat(orderAmount) / (parseFloat(trxprize)) + "(" + fromcoin + ")"}</InputAdornment>}
                        label="Amount"
                      />
                    </FormControl>
                  </div>
            ...
            <DialogActions>
              <Button id="paybutton" onClick={handlePaymentOpen}>
                Order Review
              </Button>
            </DialogActions>
          </BootstrapDialog>
          ...
          <BootstrapDialog
            ...
            open={paymentOpen}
          >
            <BootstrapDialogTitle id="customizeddd-dialog-title" onClose={handlePaymentClose}>
                  ...
                  <CardActionArea>
                    <CardContent>
                      <Typography gutterBottom variant="h6" component="div">
                        <span style={{ textAlign: "left" }}>Amount You Pay </span> <br />
                        <span style={{ textAlign: "left" }}>₹ {orderAmount}</span>
                      </Typography>
                      <Typography gutterBottom variant="h6" component="div">
                        <span style={{ textAlign: "left" }}>Coins You Get </span><br /><span style={{ textAlign: "left" }}>
                          {parseFloat(orderAmount) / (parseFloat(trxprize)) + "(" + fromcoin + ")"}
                        </span>
                      </Typography>
                    </CardContent>
                  </CardActionArea>
                  <CardActions>
                    <button id="paybutton" disabled={loading} onClick={loadRazorpay}>
                      Continue To Pay
                    </button>
                  </CardActions>
                </Card>
          ...
          <BootstrapDialog
            ...
            open={sellOpen}
          >
            <BootstrapDialogTitle id="customizeddd-dialog-title" onClose={handleSellSideClose}>
              {sellCoin}
            </BootstrapDialogTitle>
            <DialogContent dividers>
              <Typography gutterBottom>
                {/* <img src={walletIcone} style={{ height: "20px", width: "20px" }} /> */}
                <span style={{ paddingLeft: "5px" }}>  No. Of Coins You Want to Sell : </span><span style={{ color: 'blue' }}>{sellCoinAmount}</span>
              </Typography>
              <Typography gutterBottom>
                <Box sx={{ display: 'flex', flexWrap: 'wrap' }}>
                  <div>
                    <FormControl fullWidth sx={{ m: 1 }}>
                      <InputLabel htmlFor="outlined-adornment-amount">Coins</InputLabel>
                      <OutlinedInput
                        id="outlined-adornment-amount"
                        type="number"
                        value={sellCoinAmount}
                        onChange={handleSellCoinChange}
                        endAdornment={<InputAdornment position="end">{sellCoin}</InputAdornment>}
                        label="Amount"
                      />
                    </FormControl>
                    <Box sx={{ display: 'flex' }}>
                      <Typography gutterBottom>
                        ...
                        <span style={{ paddingLeft: "5px" }}>  Current Available Bal. : {reCoin !== undefined ? reCoin >= 0 ? reCoin : <span style={{ color: 'red' }}>Insufficient Balance</span> : coinAmount}</span>
                      </Typography>
                    </Box>
                    <FormControl fullWidth sx={{ m: 1 }}>
                      ...
                      <OutlinedInput
                        id="outlined-adornment-amount"
                        type="number"
                        startAdornment={<InputAdornment position="start">₹{sellCoinAmount * scoinPrizeI}</InputAdornment>}
                        label="Amount"
                      />
                    </FormControl>
                  </div>
          ...
          <BootstrapDialog
            ...
            open={orderROpen}
          >
            <BootstrapDialogTitle id="customizeddd-dialog-title" onClose={handleOrderRClose}>
              Order Review
            </BootstrapDialogTitle>
            <DialogContent dividers>
              <Typography gutterBottom>
                <Card sx={{ maxWidth: 445, border: "1px solid green" }}>
                  <CardActionArea>
                    <CardContent>
                      ...
                      <Typography gutterBottom variant="h6" component="div">
                        <span>Amount You Get (INR) </span><br /><span>
                          ₹ {sellCoinAmount * scoinPrizeI}</span>
                      </Typography>
                    </CardContent>
                  </CardActionArea>
                  <CardActions>
                    <button id="sellbutton" disabled={loading} onClick={SellCoinHandler}>
                      Ready To Sell
                    </button>
                  </CardActions>
                </Card>
